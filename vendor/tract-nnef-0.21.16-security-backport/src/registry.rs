...
impl PrimitiveDecl {
    pub fn validate(&self) -> TractResult<()> {
        self.decl
            .validate()
            .with_context(|| format!("Invalid primitive `{}'", self.decl.id.0))
    }
...
impl Registry {
    ...
    pub fn register_primitive(
        ...
        let primitive_decl = PrimitiveDecl {
            decl,
            docstrings: None,
            to_tract: func,
        };
    ...
    pub fn register_unit_element_wise(&mut self, id: impl AsRef<str>, ew: &dyn ElementWiseMiniOp) {
        ...
        self.unit_element_wise_ops
            .push((id.as_ref().into(), clone_box(ew)));
    }
    ...
    pub fn register_element_wise(
        ...
        self.element_wise_ops
            .push((id.as_ref().into(), type_id, dumper, parameters, loader));
    }
    ...
    pub fn deserialize(
        ...
        if let Some(ew) = self
            .unit_element_wise_ops
            .iter()
            .find(|ew| ew.0 == invocation.id)
        {
            let input = invocation.arguments[0]
                .rvalue
                .resolve(builder, &[])?
                .to::<OutletId>(builder)?;
            ...
            .find(|ew| ew.0 == invocation.id)
        {
            ...
            return Ok(Some((ew.4)(builder, &resolved).with_context(|| {
                format!("Deserializing op `{}'", invocation.id.0)
            })?));
        }
        if let Some(bin) = self.binary_ops.iter().find(|bin| bin.0 == invocation.id) {
            let mut a = invocation.arguments[0]
                .rvalue
                .resolve(builder, &[])?
                .to::<OutletId>(builder)?;
            let mut b = invocation.arguments[1]
                .rvalue
                .resolve(builder, &[])?
                .to::<OutletId>(builder)?;
            ...
            if !a_dt.is_quantized() || !b_dt.is_quantized() {
                let operating_dt = if a_dt == b_dt
                    && bin
                        .1
                        .operating_datum_type(a_dt, b_dt)
                        .map(|it| it == a_dt)
                        .unwrap_or(false)
                {
                    a_dt
                ...
                // assume scalar are inline and we should not trust their DT
                    ...
                    .model
                    .node(a.node)
                    .op_is::<tract_core::ops::konst::Const>()
                {
                    b_dt
                } else if builder
                    .model
...
pub fn multi_rank_broadcast(
    ...
    (inputs.iter())
        .zip(ranks.iter())
        .map(|(&i, &r)| {
            (r..max_rank).try_fold(i, |w, n| {
                Ok(builder.wire_as_outlets(AxisOp::Add(n), &[w])?[0])
            })
        })
        .collect()
}

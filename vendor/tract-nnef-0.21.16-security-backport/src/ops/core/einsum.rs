...
pub fn de_einsum(
    ...
    let expr = invocation
        .named_arg_as::<String>(builder, "expr")?
        .parse::<AxesMapping>()?;
...
pub fn de_einsum_q(
    ...
    let operating_dt = invocation.named_arg_as::<String>(builder, "acc")?;
    let operating_dt = operating_dt.parse()?;
    let output_dt = if let Some(odt) = invocation
        .get_named_arg_as::<String>(builder, "output")?
        .filter(|odt| odt.len() > 0)
    {
        odt.parse()?
    } else {

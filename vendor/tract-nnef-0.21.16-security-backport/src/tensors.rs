...
mod test {
    ...
    #[test]
    fn rejects_shape_products_that_overflow_usize() -> TractResult<()> {
        let mut header = Header::default();
        header.rank = 3;
        header.dims[0] = u32::MAX;
        header.dims[1] = u32::MAX;
        header.dims[2] = 3;
        header.bits_per_item = 32;
        header.item_type = 0;

        let mut buffer = Vec::new();
        header.write(&mut buffer)?;
        let error = read_tensor(buffer.as_slice()).expect_err("overflowing NNEF shape must fail");

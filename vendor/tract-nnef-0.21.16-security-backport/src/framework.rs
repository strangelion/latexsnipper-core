...
impl Nnef {
    ...
    fn _write_to_tar<W: std::io::Write>(
        ...
        if let Some(mut quantization) = proto_model.quantization {
            ...
            header
                .set_path("graph.quant")
                .context("Setting graph.quant path")?;
            header.set_size(quant_data.len() as u64);
            header.set_mode(0o644);
            header.set_mtime(timestamp.as_secs());
            header.set_cksum();
            ar.append(&header, &mut &*quant_data)
                .context("Appending graph.quant")?;
        }
...
fn proto_model_from_resources(
    ...
    let mut new_resources = if sub_models.len() > 0 {
        sub_models
            .into_iter()
            .try_fold(resources, |r, it| -> TractResult<HashMap<_, _>> {
                ...
                let typed_model = nnef().model_for_proto_model(
                    &proto_model_from_resources(submodel_resources).unwrap(),
                )?;
    ...
    let doc = new_resources
        ...
        .with_context(|| {
            anyhow!(
                "Resource {} was not found in the model",
                crate::resource::GRAPH_NNEF_FILENAME
            )
        })?
...
fn read_stream<R: std::io::Read>(
    ...
    if path
        .components()
        .any(|name| name.as_os_str().as_bytes().first() == Some(&b'.'))
    {
        return Ok(());
    }
    ...
    for loader in framework.resource_loaders.iter() {
        ...
        let loaded = loader.try_load(path, reader, framework).with_context(|| {
            anyhow!(
                "Error while loading resource by {:?} at path {:?}",
                loader.name(),
                path
            )
        })?;

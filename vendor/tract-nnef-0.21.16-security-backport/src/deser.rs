...
impl<'mb> ModelBuilder<'mb> {
    ...
    fn translate(&mut self) -> TractResult<()> {
            ...
            match &*ext.0 .0 {
                "tract_registry" => {
                        ...
                        .any(|reg| reg.id == registry)
                    {
                        ...
                        .registries
                        .iter()
                        .find(|reg| reg.aliases.contains(&registry))
                    {
        ...
        self.wire_body(&self.proto_model.doc.graph_def.body)
            .context("Wiring root graph body")?;
        ...
        for (ix, name) in self.proto_model.doc.graph_def.results.iter().enumerate() {
            self.model
                .set_outlet_label(outputs[ix], name.0.to_string())?;
        }
        ...
        for assignment in body {
            ...
            for (id, outlet) in identifiers.iter().zip(values.iter()) {
                self.scopes
                    .last_mut()
                    .unwrap()
                    .insert((*id).clone(), Value::Wire(*outlet));
            }
    ...
    pub fn wire_fragment_invocation(
        ...
        for par in invocation.default_params.iter() {
            inner_scope.insert(
                par.id.clone(),
                invocation.named_arg_as::<Value>(self, &par.id.0)?,
            );
        }
        ...
        if self.model.nodes().iter().any(|n| n.name == name) {
            for i in 0.. {
                ...
                if !self
                    ...
                    .nodes()
                    .iter()
                    .any(|n| n.name.starts_with(&candidate))
                {
                    return candidate;
                }
            }
    ...
    pub fn wire_as_outlets(
        ...
        self.model
            .wire_node(name, op, inputs)
            .with_context(|| format!("inputs are {inputs:?}"))
    }
...
impl ResolvedInvocation<'_> {
    ...
    pub fn named_arg(&self, name: &str) -> TractResult<Cow<'_, RValue>> {
        self.get_named_arg(name)
            .ok_or_else(|| format_err!("expected argument {}", name))
    }
    ...
    pub fn get_named_arg(&self, name: &str) -> Option<Cow<'_, RValue>> {
        ...
        if let Some((ix, param)) = self
            .default_params
            .iter()
            .enumerate()
            .find(|(_ix, param)| &*param.id.0 == name)
        {
    ...
    where
        T: CoerceFrom<Value>,
    {
        let Some(rv) = self.get_named_arg(name) else {
            return Ok(None);
        };
        let v = rv
            .resolve(builder, &[])
            ...
            RValue::Binary(left, op, right) => {
                ...
                let inv = Invocation {
                    ...
                    arguments: vec![
                        Argument {
                            id: None,
                            rvalue: left.as_ref().clone(),
                        },
                        Argument {
                            id: None,
                            rvalue: right.as_ref().clone(),
                        },
                    ],
                };
                builder
            ...
            RValue::Tuple(array) => {
                let dt_iter: Box<dyn Iterator<Item = &Option<DatumType>>> =
                    if dt.len() == 0 || dt.len() == 1 && dt[0].is_none() {
                        ...
                        bail!(
                            "Wrong number of types for a tuple, got {:?} for {:?}",
                            dt,
                            array
                        )
                    };

...
pub fn parse_quantization(doc: &str) -> TractResult<Vec<(Identifier, QuantFormat)>> {
    all_consuming(many0(quantization))(doc)
        .map(|pair| pair.1)
...
fn quantization(i: &str) -> IResult<&str, (Identifier, QuantFormat)> {
    let (i, _) = stag("")(i)?;
    let (i, id) = alt((
        delimited(tag("\""), direct_identifier, tag("\"")),
        escaped_identifier,
    ))(i)?;
    let (i, _) = stag(":")(i)?;
    let (i, qp) = qparam(i)?;
...
fn qparam(i: &str) -> IResult<&str, QuantFormat> {
    ...
    let (i, params, bits, signed) = match id {
        "linear_quantize" => {
            let (i, (bits, max, min)) = permutation((
...
mod test {
    ...
    fn test_quantization() {
        assert_eq!(
            p(
                quantization,
                r#""tensor_name": linear_quantize(min = 0.5, max = 123.8, bits = 8);"#
            ),
            (
                "tensor_name".into(),
                QuantFormat::Linear {

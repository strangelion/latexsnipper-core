...
pub fn parse_fragment_decl(doc: &str) -> TractResult<FragmentDecl> {
    all_consuming(fragment_decl)(doc)
        .map(|pair| pair.1)
        .map_err(translate_error)
}
...
pub fn parse_parameters(doc: &str) -> TractResult<Vec<Parameter>> {
    all_consuming(parameter_list)(doc)
        .map(|pair| pair.1)
        .map_err(translate_error)
}
...
fn fragment_def(i: &str) -> IResult<&str, FragmentDef> {
    spaced(map(
        pair(
            fragment_decl,
            alt((map(body, Some), map(stag(";"), |_| None))),
        ),
...
fn literal_expr(i: &str) -> IResult<&str, Literal> {
    spaced(alt((
        literal,
        map(
            delimited(stag("["), separated_list0(stag(","), literal), stag("]")),
            Literal::Array,
        ),
        map(
            delimited(stag("("), separated_list0(stag(","), literal), stag(")")),
...
fn type_spec(i: &str) -> IResult<&str, TypeSpec> {
    fn non_array_type(i: &str) -> IResult<&str, TypeSpec> {
        alt((
            tuple_type_spec,
            map(type_name, TypeSpec::Single),
            tensor_type_spec,
        ))(i)
    }
    alt((
        (map(
            terminated(non_array_type, pair(stag("["), stag("]"))),
            |t| TypeSpec::Array(Box::new(t)),
        )),
        non_array_type,
    ))(i)
}
...
fn tensor_type_spec(i: &str) -> IResult<&str, TypeSpec> {
    map(
        delimited(pair(stag("tensor"), stag("<")), type_name, stag(">")),
        TypeSpec::Tensor,
    )(i)
}
...
fn tuple_type_spec(i: &str) -> IResult<&str, TypeSpec> {
    map(
        delimited(stag("("), separated_list0(stag(","), type_spec), stag(")")),
        TypeSpec::Tuple,
    )(i)
}
...
fn assignment(i: &str) -> IResult<&str, Assignment> {
    spaced(terminated(
        map(
            separated_pair(lvalue, stag("="), rvalue),
            |(left, right)| Assignment { left, right },
        ),
        stag(";"),
    ))(i)
}
    ...
    fn inner_lvalue(i: &str) -> IResult<&str, LValue> {
        alt((
            map(
                delimited(
                    stag("["),
                    separated_list0(stag(","), inner_lvalue),
                    stag("]"),
                ),
                LValue::Array,
            ),
...
fn argument(i: &str) -> IResult<&str, Argument> {
    spaced(map(
        pair(opt(terminated(identifier, stag("="))), rvalue),
        |(id, rvalue)| Argument { id, rvalue },
    ))(i)
}
...
fn rvalue(i: &str) -> IResult<&str, RValue> {
    fn atom(i: &str) -> IResult<&str, RValue> {
        spaced(alt((
            ...
            map(
                pair(spaced(recognize(one_of("+-!"))), rvalue),
                |(op, rv)| RValue::Unary(op.into(), Box::new(rv)),
            ),
            map(
                delimited(tag("("), separated_list0(stag(","), rvalue), tag(")")),
                |mut rvs| {
                    if rvs.len() == 1 {
                        rvs.remove(0)
                    } else {
            ...
            map(
                pair(
                    ...
                    delimited(
                        ...
                        alt((
                            map(
                                separated_pair(opt(rvalue), stag(":"), opt(rvalue)),
    ...
    fn ite(i: &str) -> IResult<&str, RValue> {
        ...
        if i.starts_with("if") {
            ...
            Ok((
                i,
                RValue::IfThenElse(Box::new(IfThenElse {
                    cond,
                    then: leftmost,
                    otherwise,
                })),
...
fn comprehension_expr(i: &str) -> IResult<&str, Comprehension> {
    delimited(
        pair(stag("["), stag("for")),
        map(
            separated_pair(loop_iters, stag("yield"), rvalue),
            |(loop_iters, yields)| Comprehension {
                loop_iters,
                filter: None,
                yields,
            },
...
mod test {
    ...
    fn test_type_spec() {
        ...
        assert_eq!(
            p(type_spec, "tensor<scalar>[]"),
            Array(Box::new(Tensor(TypeName::Scalar)))
        );
        assert_eq!(
            p(type_spec, "(scalar,scalar[],tensor<scalar>)"),
            Tuple(vec!(
                Single(Scalar),
                Array(Box::new(Single(Scalar))),
                Tensor(Scalar)
            ))
        );
        assert_eq!(
            p(type_spec, "tensor<?>[]"),
            Array(Box::new(Tensor(TypeName::Any)))
        );
        assert_eq!(p(type_spec, "scalar[ ]"), Array(Box::new(Single(Scalar))));
        assert_eq!(
            p(type_spec, " ( scalar , scalar [ ] , tensor < scalar > ) "),
        ...
        #[cfg(feature = "complex")]
        assert_eq!(
            p(type_spec, "tensor<complex>[]"),
            Array(Box::new(Tensor(TypeName::Complex)))
        );
    }
    ...
    fn test_rvalue() {
        ...
        p(
            rvalue,
            "[for i in range_of(output_size) yield output_size[i] * sampling_rate[i]]",
        );
        p(
            rvalue,
            "scalar(2 ^ (bits - 1) - integer(symmetric) if signed else 0)",
        );
    }
    ...
    fn test_comprehenion() {
        p(
            comprehension_expr,
            "[for i in range_of(output_size) yield output_size * sampling_rate]",
        );
    }

...
impl InferenceRulesOp for Box<dyn Expansion> {
    ...
    fn to_typed(
        ...
        for (ix, o) in outputs.iter().enumerate() {
            ...
            let got = InferenceFact::from(got);
            let dimension_tensor_compatibility = expected.datum_type.concretize()
                == Some(DatumType::TDim)
                && got.datum_type.concretize() == Some(DatumType::I64)
                && expected.shape.unify(&got.shape).is_ok();
            if !dimension_tensor_compatibility && expected.clone().unify_with(&got).is_err() {

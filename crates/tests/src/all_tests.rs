...
mod asset_tests {
    ...
    fn test_base64_data_uri_strip() {
        ...
        let asset = MediaAsset {
            id: AssetId("data-uri-test".to_string()),
            format: AssetFormat::Png,
            mime_type: None,
            role: MediaRole::Photo,
    ...
    #[test]
    fn test_base64_invalid_returns_error() {
    ...
    #[test]
    fn test_asset_ref_visitor_nested_inline() {
        use latexsnipper_ast::*;
        // Create a document with a nested inline structure: Paragraph>Span>Link>Image
        let nested_img_id = AssetId("nested-img".to_string());
        let mut doc = Document::new();
        doc.assets.push(MediaAsset {

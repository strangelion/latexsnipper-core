...
impl Generator for SvgGenerator {
    fn generate(&self, tree: &RenderTree) -> Result<String> {
        let mut svg = String::from("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"400\" height=\"200\">\n");
    ...
    fn extension(&self) -> &str { "svg" }
    fn mime_type(&self) -> &str { "image/svg+xml" }
...
fn render_node(node: &RenderNode, svg: &mut String, y: i32) -> i32 {
    match node {
        RenderNode::Text(text) => {
            if !text.is_empty() {
                svg.push_str(&format!("  <text x=\"20\" y=\"{}\" font-family=\"serif\" font-size=\"14\">{}</text>\n", y, escape_xml(text)));
            }
            y + 20
        }
        RenderNode::Formula { latex, display_mode } => {

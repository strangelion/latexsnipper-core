...
fn test_complex_formulas() {
    ...
    // 基础测试
    verify_conversion("\\frac{a}{b}", "frac(a, b)", "基础分数");

    verify_conversion("x^{2}", "x^(2)", "基础上标");

    verify_conversion("\\sqrt{x}", "sqrt(x)", "基础根号");

    verify_conversion("\\alpha + \\beta", "alpha + beta", "基础希腊字母");
    ...
    verify_conversion("x^{2} + y^{2}", "x^(2) + y^(2)", "多个上标");

    verify_conversion("\\sum_{i=1}^{n} x_i", "sum_(i=1)^(n) x_(i)", "求和与下标");
    ...
    verify_conversion("\\sqrt[3]{x}", "root(3, x)", "n次根号");

    verify_conversion("\\binom{n}{k}", "binom(n, k)", "二项式系数");
    ...
    verify_conversion("e^{i\\pi} + 1 = 0", "e^(i pi) + 1 = 0", "欧拉公式");

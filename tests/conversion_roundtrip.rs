            ...
            blocks: vec![
                Block::Paragraph(ParagraphBlock {
                    inlines: vec![
                        Inline::Text(TextRun::new("The equation ")),
                        Inline::Formula(Formula {
                            source: FormulaSource::MathML(
                                "<mfrac><mi>a</mi><mi>b</mi></mfrac>".to_string(),
                            ),
                            display_mode: false,
                            confidence: 0.95,
                            source_info: None,
...
fn test_latex_input_to_all_outputs() {
    ...
    assert!(
        ...
        "LaTeX output should contain fraction"
    );
    assert!(
        latex.contains("E = mc^2"),
        "LaTeX output should contain formula"
    );
    ...
    let typst = DocumentConverter::new(OutputFormat::Typst)
        ...
        .unwrap();
    assert!(
        typst.contains("Formula Test"),
        "Typst should contain heading"
    );
    assert!(typst.contains("frac"), "Typst should contain fraction");

    // LaTeX → AST → HTML
    let html = DocumentConverter::new(OutputFormat::Html)
        .convert(&doc)
        .unwrap();
